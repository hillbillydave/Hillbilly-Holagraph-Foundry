#!/usr/bin/env python3
"""
holagraph_v1_2_full.py

Holagraph v1.2 – full 3D proof-of-concept:
...
- Plots:
    * Intensity map (x, y) at final time
    * Bead distribution projection (x, y)
    * 3D bead cloud
    * Power vs time
...
def moving_spot_pattern(Nx, Ny, dx, dy, lambda_, t, T_total):
    """
    Spot moves diagonally across the grid over time.
    """
    xs = np.arange(Nx) * dx
    ys = np.arange(Ny) * dy

    # Parametric motion from (2dx,2dy) to ( (Nx-2)dx, (Ny-2)dy )
    fx = 2 + (Nx - 4) * (t / T_total)
    fy = 2 + (Ny - 4) * (t / T_total)
...
def simulate_beads_3d_time(
    ...
    """
    pattern_schedule: function t -> (amp, phi)
        amp, phi are 1D arrays for all emitters.

    power_scale_watts: approximate watts per emitter at amp=1.0
    """
    ...
    for step in range(steps):
        ...
        # Power accounting: assume power ~ amp^2 per emitter
    ...
    E_final = field_2d_plane(x_grid, y_grid, x_emit, y_emit, amp, phi, lambda_, z_plane)
    I_final = intensity(E_final)

    power_time = np.array(power_time)  # shape (steps, 2)

    return x, y, z, I_final, power_time


# ============================================================
# PATTERN SCHEDULE (v1.2)
# ============================================================

def make_pattern_schedule(Nx, Ny, dx, dy, lambda_, T_total):
    """
    Returns a function t -> (amp, phi) that:
    - 0%–25%: single spot
    - 25%–50%: two spots
    - 50%–75%: line pattern
    - 75%–100%: moving spot
...
def run_v1_2():
    ...
    xf, yf, zf, I_final, power_time = simulate_beads_3d_time(
        x0, y0, z0_beads,
        x_emit, y_emit,
        Xg, Yg,
        lambda_, z_plane,
        ...
        z_min, z_max,
        pattern_schedule,
        power_scale_watts=0.06  # ~60 mW per emitter at amp=1
    )
    ...
    ax1.set_title("Holagraph v1.2 Intensity Map (Final Pattern)")
    ax1.set_xlabel("x (m)")
    ax1.set_ylabel("y (m)")
    ...
    plt.tight_layout()
    plt.savefig("holagraph_v1_2_full.png")
    plt.show()

    print("Holagraph v1.2 run complete. Output saved to holagraph_v1_2_full.png")
